...
<script setup>
  ...
  const listUsername = computed(() => route.params.username)
  const listSlug = computed(() => route.params.slug)
  ...
  const fetchListPosts = async () => {
    try {
      ...
      // Use UUID for fetching posts once we have the list loaded
      const listPosts = await savedListsStore.fetchListPosts(list.value.uuid, page.value, 15, {
        sort_by: sortBy,
        sort_dir: sortDir,
      })
  ...
  const removePostFromList = async (postId) => {
    try {
      await savedListsStore.removePostFromList(list.value.uuid, postId)
  ...
  const updateList = async () => {
    ...
    try {
      await savedListsStore.updateList(list.value.uuid, {
  ...
  const clearList = async () => {
    ...
    try {
      await savedListsStore.clearList(list.value.uuid)
      posts.value = []
      meta.value.total = 0
  ...
  onMounted(() => {
    if (authStore.isAuthenticated) {
      if (listUsername.value && listSlug.value) {
        fetchList()
      } else {
        router.push(localePath('/lists'))
      }
