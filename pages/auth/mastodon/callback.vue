      ...
      <div class="card-bg rounded-lg shadow-sm border border-default p-6 text-center">
        ...
        <div v-else-if="error" class="space-y-4">
          ...
          <h2 class="text-lg font-medium text-red-600 dark:text-red-400">
            {{ t('auth.mastodon_callback_error') }}
          </h2>
...
<script setup>
  ...
  const { completeMastodonLogin, error: authError } = useMastodonAuth()
  ...
  onMounted(async () => {
    ...
    if (!code || !state) {
      loading.value = false
      error.value = t('auth.mastodon_callback_error')
      return
    }

    // Complete the OAuth flow
    const result = await completeMastodonLogin(code, state)
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        if (isSafeRedirectUrl(result.returnUrl)) {
      ...
      error.value = authError.value || t('auth.mastodon_callback_error')
    }
  })
</script>
