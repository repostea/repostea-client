<template>
  <ProfileLayout active-tab="invitations">
    <div class="space-y-6">
      <!-- Invitation Stats -->
      <div
        class="card-bg rounded-lg shadow-sm border invitations-border p-6"
      >
      ...
      <!-- Invitations List -->
      <div
        class="card-bg rounded-lg shadow-sm border invitations-border"
      >
        ...
        <div v-else-if="invitations.length === 0" class="p-6 text-center text-gray-500 dark:text-gray-400">
            ...
            <div class="flex items-center justify-between">
              <div class="flex-1">
                ...
                <div class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                  <span>{{ t('invitations.uses') }}: {{ invitation.current_uses }} / {{ invitation.max_uses }}</span>
                  <span class="mx-2">•</span>
                  <span>{{ t('invitations.created') }}: {{ formatDate(invitation.created_at) }}</span>
                  <span v-if="invitation.expires_at" class="mx-2">•</span>
                  <span v-if="invitation.expires_at">{{ t('invitations.expires') }}: {{ formatDate(invitation.expires_at) }}</span>
                </div>
              </div>
...
const canCreate = computed(() => stats.value.remaining > 0 || stats.value.remaining === 'unlimited')
...
async function deleteInvitation(id) {
  ...
  try {
    await $api.invitations.deleteInvitation(id)
