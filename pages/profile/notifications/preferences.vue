    ...
    <div class="space-y-6">
      ...
      <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg p-4">
        <div class="flex items-start gap-3">
          <div class="flex-shrink-0 w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-800 flex items-center justify-center">
            <Icon name="fa6-solid:flask" class="text-amber-600 dark:text-amber-400" aria-hidden="true" />
          </div>
          <div>
            <h3 class="font-medium text-amber-800 dark:text-amber-200 flex items-center gap-2">
              {{ t('notifications.preferences.beta_title') }}
              <span class="text-xs px-2 py-0.5 rounded-full bg-amber-200 dark:bg-amber-700 text-amber-700 dark:text-amber-300">Beta</span>
            </h3>
      ...
      <div class="card-bg rounded-lg shadow-sm border prefs-border overflow-hidden">
        ...
        <div class="p-6">
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">
                {{ isSubscribed ? t('notifications.preferences.push_enabled') : t('notifications.preferences.push_disabled') }}
              </p>
            </div>
            <div class="flex items-center gap-3">
              ...
              >
                <span v-if="loading" class="inline-block animate-spin h-4 w-4 mr-2 border-2 border-white border-t-transparent rounded-full"/>
          ...
          <div v-if="isSubscribed" class="mt-4 pt-4 border-t prefs-border">
            ...
            >
              <span v-if="testLoading" class="inline-block animate-spin h-4 w-4 mr-2 border-2 border-current border-t-transparent rounded-full"/>
      ...
      <div class="card-bg rounded-lg shadow-sm border prefs-border overflow-hidden">
        ...
        <div class="p-6">
          ...
          <!-- Snooze status -->
          <div v-if="snoozeStatus.is_snoozed" class="mb-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
            <div class="flex items-center justify-between">
              <div class="flex items-center">
                <Icon name="fa6-solid:bell-slash" class="text-yellow-600 dark:text-yellow-400 mr-3" aria-hidden="true" />
                <div>
                  <p class="font-medium text-yellow-800 dark:text-yellow-200">
                    {{ t('notifications.preferences.snooze_active') }}
                  </p>
                  <p class="text-sm text-yellow-700 dark:text-yellow-300">
                    {{ t('notifications.preferences.snooze_until', { time: formatSnoozeTime(snoozeStatus.snoozed_until) }) }}
                  </p>
                </div>
              </div>
              <button
      ...
      <div class="card-bg rounded-lg shadow-sm border prefs-border overflow-hidden">
        ...
        <div class="p-6">
          ...
          <div v-if="digestFrequency !== 'none'" class="space-y-3">
            <p class="text-sm font-medium mb-2">{{ t('notifications.preferences.digest_content') }}</p>
            <label
      ...
      <div class="card-bg rounded-lg shadow-sm border prefs-border overflow-hidden">
        <div class="px-6 py-4 border-b prefs-border">
          <h2 class="text-lg font-medium">{{ t('notifications.preferences.quiet_hours_section') }}</h2>
        </div>
        <div class="p-6">
          <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
          ...
          <!-- Enable toggle -->
          <label class="flex items-center justify-between p-3 rounded-lg prefs-option-hover transition-colors cursor-pointer mb-4">
          ...
          <div v-if="quietHoursEnabled" class="mt-6 pt-4 border-t prefs-border">
            ...
            >
              <span v-if="savingQuietHours" class="inline-block animate-spin h-4 w-4 mr-2 border-2 border-white border-t-transparent rounded-full"/>
