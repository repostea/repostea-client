<template>
  <div class="container mx-auto p-2 sm:p-4 max-w-3xl">
    ...
    <div v-else class="space-y-6">
      ...
      <div class="card-bg rounded-lg shadow-sm border settings-border">
        <div class="px-6 py-4 border-b settings-border">
          ...
          <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">s/{{ sub.name }}</p>
        </div>
        ...
        <div class="p-6">
          ...
          <div v-show="activeTab === 'moderation'" role="tabpanel" class="space-y-6">
            ...
            <div v-if="sub.require_approval">
              <div class="flex items-center justify-between mb-4">
                ...
                <span
                  ...
                  class="px-2 py-0.5 text-xs bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 rounded-full"
                >
                  {{ pendingPosts.length }}
                </span>
              </div>
              ...
              <div
                ...
                class="text-center py-4 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-lg"
              >
                <Icon
                  name="fa6-solid:circle-check"
                  class="text-3xl mb-2 text-green-500"
                  aria-hidden="true"
                />
                  ...
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex-1 min-w-0">
                      ...
                      <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        {{ $t('common.by') }} {{ post.user?.username }} ·
            ...
            <div class="pt-6 border-t settings-border">
              ...
              <div
                v-else-if="hiddenPosts.length === 0"
                class="text-center py-4 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-lg"
              >
                <Icon
                  name="fa6-solid:circle-check"
                ...
                >
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex-1 min-w-0">
                      ...
                      <p
                        v-if="post.moderation_reason"
                        class="text-xs text-orange-600 dark:text-orange-400 mt-1"
                      >
                        <strong>{{ $t('subs.hidden_reason') }}:</strong>
                        {{ post.moderation_reason }}
                      </p>
                      <p
                        v-if="post.moderated_by"
          ...
          <div v-show="activeTab === 'members'" role="tabpanel" class="space-y-6">
            ...
            <div v-if="sub.is_private">
              <div class="flex items-center justify-between mb-4">
                ...
                <span
                  v-if="membershipRequests.length > 0"
                  class="px-2 py-0.5 text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full"
                >
              ...
              <div
                v-else-if="membershipRequests.length === 0"
                class="text-center py-4 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-lg"
              >
                <Icon
                  name="fa6-solid:circle-check"
                ...
                >
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex items-center gap-3">
                      ...
                      <div class="flex-1 min-w-0">
                        <NuxtLink
                          :to="localePath(`/u/${request.username}`)"
                          class="font-medium text-sm hover:text-primary"
                        >
                          {{ request.username }}
                        </NuxtLink>
                        <p
                          v-if="request.pivot?.request_message"
          ...
          <div v-show="activeTab === 'team'" role="tabpanel">
            ...
            <div v-else class="space-y-2">
              ...
              <div
                class="flex items-center justify-between p-2 rounded-lg bg-primary/5 border border-primary/20"
              >
                <div class="flex items-center gap-3">
                  <img
              ...
              >
                ...
                <button
                  ...
                  @click="openRemoveModeratorConfirm(mod)"
                >
                  <Icon name="fa6-solid:user-minus" aria-hidden="true" />
                </button>
              </div>
      ...
      <Teleport to="body">
        <div
          ...
          @click.self="showAddModeratorModal = false"
        >
          <div
            class="card-bg rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[80vh] overflow-hidden"
          >
            <div class="px-6 py-4 border-b settings-border">
              <h3 class="text-lg font-bold">{{ $t('subs.add_moderator') }}</h3>
              <p class="text-sm text-gray-500 dark:text-gray-400">
                {{ $t('subs.add_moderator_description') }}
              </p>
            </div>
            <div class="p-6 max-h-[50vh] overflow-y-auto">
              ...
              <div v-else class="space-y-2">
                <div
                  ...
                  :class="
                    selectedMemberForMod?.id === member.id
    ...
    <ConfirmModal
      ...
      :title="t('subs.remove_member_title')"
      :message="t('subs.confirm_remove_member', { username: pendingRemoveMember?.username || '' })"
    ...
    <ConfirmModal
      v-model="showRemoveModeratorConfirm"
      :title="t('subs.remove_moderator_title')"
      :message="
        t('subs.confirm_remove_moderator', { username: pendingRemoveModerator?.username || '' })
      "
      :confirm-text="t('common.remove')"
      :loading="removingModerator !== null"
      variant="danger"
      @confirm="removeModerator"
    />
  </div>
</template>
...
<script setup>
  ...
  const showRemoveModeratorConfirm = ref(false)
  const pendingRemoveModerator = ref(null)
  ...
  const tabs = computed(() => {
    const baseTabs = [
      ...
      {
        id: 'moderation',
        label: t('subs.settings_tab_moderation'),
    ...
    if (sub.value?.is_owner) {
      baseTabs.push({
        id: 'team',
        label: t('subs.settings_tab_team'),
  ...
  function initForm() {
    if (sub.value) {
      form.value = {
        ...
        allowed_content_types: sub.value.allowed_content_types || [
          'text',
          'link',
          'image',
          'video',
  ...
  function openRemoveMemberConfirm(member) {
  ...
  async function removeMember() {
    ...
    removingMember.value = pendingRemoveMember.value.id
    try {
      await $api.subs.removeMember(sub.value.id, pendingRemoveMember.value.id)
      members.value = members.value.filter((m) => m.id !== pendingRemoveMember.value.id)
  ...
  function openRemoveModeratorConfirm(moderator) {
    pendingRemoveModerator.value = moderator
    showRemoveModeratorConfirm.value = true
  }

  async function removeModerator() {
    if (!pendingRemoveModerator.value) return

    removingModerator.value = pendingRemoveModerator.value.id
    try {
      await $api.subs.removeModerator(sub.value.id, pendingRemoveModerator.value.id)
      moderators.value = moderators.value.filter((m) => m.id !== pendingRemoveModerator.value.id)
      showRemoveModeratorConfirm.value = false
      pendingRemoveModerator.value = null
