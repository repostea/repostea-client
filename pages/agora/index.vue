  ...
  <div class="container mx-auto px-1 md:px-4 py-2 md:py-4">
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center gap-2">
        <div
          v-if="errorType === 'connection_limit'"
          class="group relative"
        >
          <span class="flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400 cursor-help">
            <span class="w-1.5 h-1.5 bg-amber-500 rounded-full"/>
            {{ $t('agora.server_busy') }}
          </span>
          <div class="absolute right-0 top-full mt-2 w-64 p-3 bg-amber-50 dark:bg-amber-900/90 border border-amber-300 dark:border-amber-700 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20">
            <p class="text-xs text-amber-700 dark:text-amber-300">{{ t('agora.offline_explanation') }}</p>
            <button
        ...
        >
          ...
          <div class="absolute right-0 top-full mt-2 w-64 p-3 bg-red-50 dark:bg-red-900/90 border border-red-300 dark:border-red-700 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20">
            <p class="text-xs text-red-700 dark:text-red-300">{{ t('agora.offline_explanation') }}</p>
            <button
    ...
    <ClientOnly>
      ...
      <div v-if="authStore.user" class="mb-4">
        ...
        >
          ...
          />
          <div v-else class="w-8 h-8 rounded-full agora-avatar-placeholder flex items-center justify-center flex-shrink-0">
            <Icon name="fa6-solid:user" class="text-gray-400 text-sm" aria-hidden="true" />
          </div>
          <!-- Fake input with integrated button on mobile -->
          <div class="agora-compose-input flex-1 px-3 py-2 rounded-lg text-sm flex items-center justify-between">
        ...
        <Teleport to="body" :disabled="!isMobileComposeModal">
          <div
            v-if="showCompose"
            :class="isMobileComposeModal ? 'fixed inset-0 z-50 flex items-end sm:items-center justify-center' : ''"
          >
            <!-- Backdrop for mobile -->
            <div
                ...
                <div>
                  <div class="flex flex-wrap items-center gap-1 text-sm agora-text-muted">
                    ...
                    <div class="relative inline-block">
                      ...
                      >
                        <button
                          ...
                          :class="expiresInHours === option.value ? 'text-primary font-medium' : ''"
                    ...
                    <div class="relative inline-block">
                      ...
                      >
                        {{ expiryMode === 'from_last' ? t('agora.expiry_mode_from_last').toLowerCase() : t('agora.expiry_mode_from_first').toLowerCase() }}
                        <Icon name="fa6-solid:chevron-down" class="text-[10px]" />
                      </button>
                      <div
      ...
      >
        ...
        <div v-if="!showLoginForm" class="flex flex-wrap items-center justify-center gap-2">
          ...
          <span v-if="mastodonEnabled || mbinEnabled" class="text-gray-400 dark:text-gray-500 text-xs">{{ t('common.or') }}</span>
          <button
        ...
        <div v-else-if="showLoginForm === 'mastodon'" class="space-y-3">
          <div class="flex items-center justify-between">
            <span class="text-sm font-medium flex items-center gap-2 text-gray-700 dark:text-gray-300">
        ...
        <div v-else-if="showLoginForm === 'mbin'" class="space-y-3">
          ...
          <div
            ...
            class="text-xs text-red-600 dark:text-red-400"
          >
            {{ mbinError === 'instance_forbidden' ? t('auth.mbin_error_403') : mbinError === 'connection_failed' ? t('auth.mbin_error_instance') : mbinError }}
          </div>
        </div>
      </div>
    </ClientOnly>
    ...
    <div class="space-y-4">
      ...
      <div v-else-if="error" class="agora-error-state rounded-xl p-8 text-center">
        <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center">
          <Icon name="fa6-solid:circle-exclamation" class="text-2xl text-red-500" aria-hidden="true" />
        </div>
      ...
      <!-- Empty state -->
      <div v-else-if="messages.length === 0" class="agora-empty-state rounded-xl p-12 text-center relative overflow-hidden">
        <div class="relative z-10">
          <div class="w-20 h-20 mx-auto mb-4 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center">
        ...
        >
          ...
          >
            ...
            <span class="font-medium text-gray-700 dark:text-gray-300">
              {{ message.parent?.is_anonymous ? t('agora.anonymous_user') : ('@' + message.parent?.user?.username) }}
            </span>
