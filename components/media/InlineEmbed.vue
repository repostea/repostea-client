<template>
  <div ref="containerRef" class="inline-embed my-3">
    ...
    <div v-if="!isVisible" class="embed-placeholder rounded-lg overflow-hidden border">
...
<script setup lang="ts">
  ...
  const isAudio = computed(() => {
    return (
      props.url.includes('spotify.com') ||
      props.url.includes('soundcloud.com') ||
      props.url.includes('suno.com')
    )
  })
  ...
  const embedTwitter = () => {
    ...
    ;(window as any).twttr.widgets.createTweet(tweetId, twitterContainer.value, {
  ...
  const loadInstagramWidget = () => {
    ...
    instagramContainer.value.appendChild(blockquote)
