  ...
  >
    <div class="flex items-center justify-center gap-1.5 px-1 py-1">
      ...
      <div v-if="federation.likes_count" class="flex items-center gap-0.5">
        ...
        <span class="text-[10px] tabular-nums font-medium">{{ formatNumber(federation.likes_count) }}</span>
      </div>
      <!-- Shares -->
      <div v-if="federation.shares_count" class="flex items-center gap-0.5">
        ...
        <span class="text-[10px] tabular-nums font-medium">{{ formatNumber(federation.shares_count) }}</span>
      </div>
    </div>
          ...
          <div class="federation-modal rounded-lg shadow-xl max-w-sm w-full p-5" @click.stop>
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
              <div class="stat-box text-center p-3 rounded-lg">
                <div class="flex items-center justify-center gap-1 text-xl font-bold tabular-nums stat-value">
                  <Icon name="fa6-solid:heart" class="text-base" aria-hidden="true" />
                  {{ federation.likes_count || 0 }}
                </div>
                <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $t('federation.likes') }}</div>
              </div>
              <div class="stat-box text-center p-3 rounded-lg">
                <div class="flex items-center justify-center gap-1 text-xl font-bold tabular-nums stat-value">
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $t('federation.shares') }}</div>
              </div>
            </div>
