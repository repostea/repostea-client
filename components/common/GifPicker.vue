<template>
  <div class="relative">
    ...
    >
      ...
      <div
        ref="gridContainer"
        class="gif-grid flex-1 overflow-y-auto"
        @scroll="handleScroll"
      >
        <!-- Loading state -->
        ...
        <div v-else-if="error" class="text-center py-8">
          <Icon name="fa6-solid:circle-exclamation" class="text-2xl text-red-500 mb-2" aria-hidden="true" />
        ...
        <div v-else-if="gifs.length === 0 && !isLoading" class="text-center py-8">
          <Icon name="fa6-regular:face-meh" class="text-3xl text-text-muted dark:text-text-dark-muted mb-2" aria-hidden="true" />
          <p class="text-sm text-text-muted dark:text-text-dark-muted">{{ t('gifs.no_results') }}</p>
        </div>
...
function handleClickOutside(event) {
  if (isOpen.value && !event.target.closest('.gif-picker') && !event.target.closest('.gif-toggle-btn')) {
    isOpen.value = false
    clear()
  }
}
