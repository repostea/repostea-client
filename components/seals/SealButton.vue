<template>
  <div class="seal-button-container">
    ...
    <div
      v-if="showAvailable && userSeals"
      class="seals-available"
      :title="$t('seals.available_tooltip')"
    >
  ...
  const toggleSeal = async (type: 'recommended' | 'advise_against') => {
    ...
    try {
      ...
      if (response.success) {
        ...
        success(isRemoving ? t('seals.seal_removed_success') : t('seals.seal_applied_success'), {
          priority: 'low',
        })
      }
    } catch (err: any) {
      ...
      if (!err._interceptorWillNotify) {
        const message =
          err.response?.data?.error || err.response?.data?.message || t('seals.seal_error')
        error(message)
      }
    }
  }
